...
public class triggerCuadro1 : MonoBehaviour {
	...
	void Start () {
		...
		client.Subscribe(new string[] { "museo/cuadro1" }, new byte[] { MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE }); 
		
		client.Publish("museo/cuadro1", System.Text.Encoding.UTF8.GetBytes("cuadro1"), MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE, true);
	...
	void Update () {
		...
		if(topicNuevo.Equals("cuadro1")){
			rend.sharedMaterial = material[0];
			Debug.Log("CAMBIO A LIBRO 1");

		}else if(topicNuevo.Equals("cuadro2")){
			rend.sharedMaterial = material[1];
			Debug.Log("CAMBIO A LIBRO 2");
		}else if(topicNuevo.Equals("cuadro3")){
...
/* 		if(topic.Equals("RFID1 ON")){
	...
	void OnTriggerEnter(Collider c){
		 ...
		 client.Publish("museo/androidCall", System.Text.Encoding.UTF8.GetBytes("cuadroAndroid"), MqttMsgBase.QOS_LEVEL_EXACTLY_ONCE, true);
         }else{ 
